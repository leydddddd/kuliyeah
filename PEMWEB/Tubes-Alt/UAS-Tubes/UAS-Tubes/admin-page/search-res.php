        ...
        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
            <?php if ($row['source'] === 'news'): ?>
                ...
                <div class="row align-items-center border p-3 shadow-sm rounded mb-3">
                    <!-- Gambar -->
                    <div class="col-2">
                        <img src="./page/uploads/thumbnails/<?= htmlspecialchars('thumb_' . $row['image']) ?>" class="img-fluid rounded" alt="<?= htmlspecialchars($row['title']) ?>" />
                    </div>
                    <!-- Judul & Tanggal -->
                    <div class="col-8">
            ...
            <?php elseif ($row['source'] === 'guide'): ?>
                ...
                <div class="row align-items-center border p-3 shadow-sm rounded mb-3">
                    ...
                    <!-- Aksi -->
                    <div class="col-2 text-end">
                        <a href="./page/guide/editGuidePost.php?id=<?= $row['id'] ?>" class="btn btn-lg"><i class="bi bi-scissors"></i></a>
                        <a href="./page/guide/processDeleteGuide.php?id=<?= $row['id'] ?>" class="btn btn-lg" onclick="return confirm('Yakin ingin menghapus?')"><i class="bi bi-trash"></i></a>
                        <a href="./page/guide/guide.php?id=<?= $row['id'] ?>" class="btn btn-lg"><i class="bi bi-eye"></i></a>
                    </div>
                </div>
